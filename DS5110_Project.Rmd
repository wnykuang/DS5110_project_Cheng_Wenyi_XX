...
```{r, Warning=FALSE}
lm_fit <- lm(price ~ bedrooms + room_type + Air_Conditioning + Elevator_in_Building + accommodates, data = Training)
summary(lm_fit)
```
...
```{r}
x <- model.matrix(price ~ bedrooms + room_type + Air_Conditioning + Elevator_in_Building + accommodates, Training)[,-1]
y <- Training$price
...
```{r}
x <- model.matrix(price ~ bedrooms + room_type + Air_Conditioning + Elevator_in_Building + accommodates, listings)[-ratio, -1]
